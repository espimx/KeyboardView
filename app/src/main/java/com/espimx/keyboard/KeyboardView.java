...
import static com.espimx.keyboard.Keyboard.*;
import static com.espimx.keyboard.Utils.dp2px;
...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    private boolean mKeyTextBold;

    private TextView mPreviewText;
    private PopupWindow mPreviewPopup;  //尽量不使用预览Popup
    private int mPreviewTextSizeLarge;
    private int mPreviewOffset;
    private int mPreviewHeight = 80;
    // Working variable
    ...
    private int mKeyBackgroundShadowColor;
    private float mKeyBackgroundShadowRadius;
...
//        mPreviewHeight = a.getDimensionPixelSize(R.styleable.KeyboardView_keyPreviewHeight, 80);
        mKeyTextSize = a.getDimensionPixelSize(R.styleable.KeyboardView_keyTextSize, dp2px(context, 18));
        mKeyTextColor = a.getColor(R.styleable.KeyboardView_keyTextColor, 0xFF000000);
        mLabelTextSize = a.getDimensionPixelSize(R.styleable.KeyboardView_labelTextSize, dp2px(context, 20));
        mPopupLayout = a.getResourceId(R.styleable.KeyboardView_popupLayout, 0);
        mShadowColor = a.getColor(R.styleable.KeyboardView_shadowColor, 0);
        mShadowRadius = a.getFloat(R.styleable.KeyboardView_shadowRadius, 0f);
        mKeyTextBold = a.getBoolean(R.styleable.KeyboardView_keyTextBold, false);
        mKeyBackgroundShadowColor = a.getColor(R.styleable.KeyboardView_keyBackgroundShadowColor, Color.parseColor("#33000000"));
        mKeyBackgroundShadowRadius = a.getFloat(R.styleable.KeyboardView_keyBackgroundShadowRadius, 0f);
    ...
    private void onBufferDraw() {
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            if (mKeyBackgroundShadowRadius > 0) {
                paint.setColor(mKeyBackgroundShadowColor);
                paint.setMaskFilter(new BlurMaskFilter(mKeyBackgroundShadowRadius, BlurMaskFilter.Blur.NORMAL));
                canvas.drawRect(0, 5, key.width, key.height, paint);
            }
            ...
            if (label != null) {
                ...
                if (label.length() > 1 && key.codes.length < 2) {
                    ...
                    paint.setTypeface(mKeyTextBold ? Typeface.DEFAULT_BOLD : Typeface.DEFAULT);
                }
    ...
    private void detectAndSendKey(int index, int x, int y, long eventTime) {
        if (index != NOT_A_KEY && index < mKeys.length) {
            ...
            if (key.text != null) {
                ...
                if (mInMultiTap) {
                    if (mTapCount != -1) {
                        mKeyboardActionListener.onKey(KEYCODE_DELETE, KEY_DELETE);
                    } else {
                        mTapCount = 0;
                    }
